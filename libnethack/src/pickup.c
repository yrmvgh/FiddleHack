...
in_container(struct obj *obj)
{
    ...
    */
    ...
    if (obj->otyp == ICE_BOX || Is_box(obj) || obj->otyp == BOULDER ||
        ...
        pline(msgc_failcurse, "You cannot fit %s into %s.", the(xname(obj)),
...
use_container(struct obj *obj, int held)
{
    ...
    if (obj->olocked) {
        ...
        chest_trap(&youmonst, obj, HAND, FALSE);
...
menu_loot(int retry, struct obj *container, boolean put_in)
{
    ...
    if (loot_everything) {
        ...
        otmp = put_in ? youmonst.minvent : cobj_is_magic_chest(container) ?
