    ...
    if (!unid_cnt) {
        ...
        if (!you && vis && mon->mtame)
            pline(msgc_youdiscover,
                  "%s shares %snewly identified object%s from %s inventory:",
        ...
        if (!you) {
            if (vis && mon->mtame)
                pline(msgc_info,
                      "%s shares %snewly learned object%s from %s inventory:",
                      Monnam(mon), unid_cnt == 1 ? "a " : "",
                      unid_cnt == 1 ? "" : "s", mhis(mon));
            n = 0;
...
display_pickinv(const char *lets, boolean want_reply, long *out_cnt)
{
    ...
    wtcap = weight_cap();
    wt = inv_weight_total();
    invslots = inv_cnt(TRUE);

    const char *invbuf;
    invbuf = msgprintf("Inventory: %d/%dwt (%d/52 slots)",
                       wt, wtcap, invslots);

    if (objlist.icount)
        n = display_objects(&objlist, want_reply ? NULL : invbuf,
...
useupf(struct obj *obj, long numused)
{
    ...
    if (obj->quan > numused)
        ...
        otmp = obj;
    /* if the player caused the useupf, charge for it */
    if (!flags.mon_moving && costly_spot(otmp->ox, otmp->oy)) {
