...
resolve_uim(enum u_interaction_mode uim, boolean weird_attack, xchar x, xchar y)
{
    ...
    if (!last_command_was("moveonly")) {
        ...
        if (l->mem_bg >= S_stone && l->mem_bg <= S_trwall &&
            bad_rock(&youmonst, x, y) && (!uwep || !is_pick(uwep))) {
            if (!cansee(x, y))
       ...
       enum occupation thismove)
{
    ...
    if (Engulfed) {
        ...
        /* check slippery ice */
        if ((fumbling(u.usteed ? u.usteed : &youmonst) & W_MASK(os_circumstance)) &&
            !rn2(resists_cold(u.usteed ? u.usteed : &youmonst) ? 3 : 2))
            set_property(u.usteed ? u.usteed : &youmonst, FUMBLING, 1, TRUE);
            ...
            if (reveal_monster_at(x, y, TRUE)) {
                if (was_mimic)
                    ...
                    pline(msgc_youdiscover,
                          "You uncover %s true location!", s_suffix(mon_nam(mtmp)));
                else
                    pline(msgc_youdiscover,
    ...
    if (uia == uia_displace && mtmp && !(m_mhiding(mtmp))) {
        ...
        if (mtmp->mtrapped && (trap = t_at(level, mtmp->mx, mtmp->my)) != 0 &&
            ...
            pline_once(mtmp->mtame ? msgc_petneutral :
                       mtmp->mpeaceful ? msgc_monneutral :
                       msgc_petfatal, "You %s %s.",
...
dopickup(const struct nh_cmd_arg *arg)
{
    ...
    if (!OBJ_AT(u.ux, u.uy)) {
        if (IS_MAGIC_CHEST(level->locations[u.ux][u.uy].typ))
            pline(msgc_cancelled, "The magic chest is firmly attached to the floor.");
        else
